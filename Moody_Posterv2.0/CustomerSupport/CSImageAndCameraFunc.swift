...
extension CustomerSupport : UIImagePickerControllerDelegate, UINavigationControllerDelegate{
    ...
    func sendImage(imageBase64:Data,index:Int){
        var indexqw = index
            if(index != 0){
                indexqw = index
            }else{
                indexqw = addImageInChat(imagData: imageBase64)
        }
        if CheckInternet.Connection() {
            ApiManager.sharedInstance.apiCaller(hasBodyData: true, hasToken: true, url: ENDPOINTS.sendTicketMessage, dictionary: createSendImageDictionary(imageBase64: imageBase64), httpMethod: "POST", token: UserDefaults.standard.string(forKey: DefaultsKeys.token)) { [self] (Result) in
            switch Result {
            case .success(let response):
            
                addImageinChat(messageId: "\(response["message_id"] as! String)", role: "\(response["role"] as! String)", imagData: imageBase64 as Data, index: indexqw,is_notified: false,time: "\(response["message_time"] as! String)")
                break
            case .failure(_):
            ...
            addMessageLocally(message: imageBase64.base64EncodedString(), type: "image", index: indexqw, localImage: imageBase64)
        }
    }
    
    //MARK: add image in chat.
    func addImageInChat(imagData:Data) -> Int{
        messages.append(Messgae(sender: checkSenderType(senderType: "is_poster"), messageId: "xx12cd", sentDate: Date(), kind: .photo(Media(image: UIImage(data: imagData)!, realImageUrl: "")), downloadURL: ""))
    ...
    //MARK: create image dictionary(json) for api calling.
    func createSendImageDictionary(imageBase64:Data)->[String:Any]{
    ...
    func sendVideo(dataOfVideo:Data, videoLink:URL){
        ...
        ApiManager.sharedInstance.apiCaller(hasBodyData: true, hasToken: true, url: ENDPOINTS.sendTicketMessage, dictionary: dictionary, httpMethod: "POST", token: UserDefaults.standard.string(forKey: DefaultsKeys.token)) { [self] (Result) in
            switch Result {
            case .success(let response):
...
//    //MARK: convert video to data and pass to sending function
            ...
            picker.didFinishPicking { [self, unowned picker] items, cancelled in
                ...
                for item in items{
                    ...
                    case .photo(p: let p):
                        ...
                        self.sendImage(imageBase64: img.jpegData(compressionQuality: 0.5)!, index: 0)
                    case .video(v: let v):
    ...
    func openCamera(){
        ...
        if(UIImagePickerController.isSourceTypeAvailable(UIImagePickerController.SourceType.camera)){
            ...
            AVCaptureDevice.requestAccess(for: AVMediaType.video) { granted in
                if(!granted && !CustomerSupport.camBool){
                    CustomerSupport.camBool = true
                    DispatchQueue.main.async {
...
//    //MARK: pick image or video from gallery or capture image from camera
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        if(picker.sourceType == .camera){
            ...
            ApiManager.sharedInstance.apiCaller(hasBodyData: true, hasToken: true, url: ENDPOINTS.sendTicketMessage, dictionary: createSendImageDictionary(imageBase64: (imageP?.jpegData(compressionQuality: 0.0)!)!), httpMethod: "POST", token: UserDefaults.standard.string(forKey: DefaultsKeys.token)) { [self] Result in
                switch Result {
                case .success(let response):
                    addImageinChat(messageId: response["message_id"] as! String, role: "is_poster", imagData: (img.jpegData(compressionQuality: 1.0))!, index: 0, is_notified: false, time: "\(response["message_time"] as! String)")
                    break
                case .failure(let error):
                    DispatchQueue.main.async {
                        presentAlert(title: NSLocalizedString("Message Sending Failed", comment: ""), message: NSLocalizedString( error.title, comment: ""), parentController: self)
                    ...
                    break
//                    ErrorLogging.sharedInstance.getError(APiName: ENDPOINTS.conversationUpdate, reasonOfCrash: error.title!, lineNumber: #line, functiionName: #function, fileName: #file)
//                    errorAlert()
                }
            }
        }
