...
open class BasicAudioController: NSObject, AVAudioPlayerDelegate, UIGestureRecognizerDelegate {
    ...
    open func playSound(for message: MessageType, in audioCell: AudioMessageCell) {
        ...
        case .audio(let item):
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) { [self] in
                guard let polo = try? Data(contentsOf: item.url) else{
                ...
                audioPlayer?.peakPower(forChannel: 0)
    ...
    func setAudioPlayer(completion:@escaping() -> Void){
    ...
    open func stopAnyOngoingPlaying() {
        ...
        if let cell = playingCell {
            ...
            cell.durationLabel.text = displayDelegate.audioProgressTextFormat(Float(player.duration - 1), for: cell, in: collectionView)
    ...
    @objc private func didFireProgressTimer(_ timer: Timer) {
        ...
        if let playingCellIndexPath = collectionView.indexPath(for: cell) {
            ...
            if currentMessage != nil && currentMessage?.messageId == playingMessage?.messageId {
                ...
                cell.durationLabel.isHidden = false
                cell.durationLabel.text = displayDelegate.audioProgressTextFormat(Float(player.currentTime - 1), for: cell, in: collectionView)
            } else {
