...
//  Moody_Posterv2.0
//
//  Created by Zaid Ahmed IoS on 15/10/2021.
//
...
class AKAudioRecorder: NSObject {
    ...
    private func InitialSetup(){
        ...
        audioFilename = getDocumentsDirectory().appendingPathComponent("recordingHome\(Date()).m4a")
        
        
        do{ /// Setup audio player
            try audioSession.setCategory(AVAudioSession.Category.playAndRecord, options: .defaultToSpeaker)
            audioRecorder = try AVAudioRecorder(url: audioFilename  ?? URL(fileURLWithPath: "") , settings: settings)
